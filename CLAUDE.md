...
- **adapters/** — Per-source feed adapters. `adapters/__init__.py` auto-discovers any module in the package exporting `ADAPTER` and exposes `FEED_TYPES` plus `adapt_entries(feed_type, parsed)`; `adapters/f5.py` handles the F5 Support (NGINX) feed shape (extra `copyright`/`ttl` channel fields) and **filters to security advisories only — its `entries()` drops any item whose title and description lack a `CVE-YYYY-NNNN` reference** (`CVE_RE`), so non-CVE F5 news never reaches the poller, previews, or seen-entry tracking; `adapters/royalroad.py` handles Royal Road fiction feeds (`generator` channel field, `guid` with an `isPermaLink` attribute, HTML-rich descriptions); `adapters/ubuntu.py` handles the Ubuntu Security Notices (USN) feed (`copyright`/`generator`/`docs` channel fields, `guid` with an `isPermaLink` attribute, plain-text descriptions that inline the affected CVEs) and — unlike F5 — **does not filter**, since every USN item is already a security advisory. Each feed stores a `feed_type` (default `generic` = raw feedparser entries) chosen via the `rss add` type option, and the poller parses entries through the matching adapter on every cycle.
...
- **db.py** — Async SQLite abstraction layer with a global `_conn` connection. Defines the schema and provides functions for managing feeds and tracking seen entries to prevent duplicates. `init` patches missing columns into pre-existing DBs via `PRAGMA table_info` + `ALTER TABLE`. Key tables:
  - `feeds`: One row per feed subscription, scoped by guild, tracks the parsing adapter (`feed_type`), polling metadata (etag, last_modified, last_polled), failure state (`fail_count`, `last_error`), and the embed avatar (`icon_url`). `reset_feeds(guild_id)` clears the polling cursors for one guild (backs `/rss reset`); `reset_all_feeds()` is its global counterpart with no guild filter (backs the daily calibration task).
...
- **cogs/rss.py** — The RSS management cog. Contains:
  ...
  - `calibrator` task: A second `tasks.loop` scheduled at **00:01 GMT+7** (a tz-aware `datetime.time` on a fixed `UTC+7` offset — no DST, no `tzdata` needed) that runs `db.reset_all_feeds()`, the global counterpart of `/rss reset`. It clears `last_polled`/`etag`/`last_modified` for **every** feed in **every** guild so they all re-fetch on the next cycle; `seen_entries` and `fail_count` are preserved, so only genuinely new items are announced. It is silent except for a `[calibration]` INFO log line. Started in `cog_load` / cancelled in `cog_unload` alongside `poller`; `before_loop` waits for the bot to be ready.
...
- `[calibration] reset last-poll for <n> feed(s) across all guilds` — once per day at 00:01 GMT+7 when the calibration task re-polls every feed
