...
async def reset_all_feeds() -> int:
    ...
    The global counterpart of reset_feeds(): clears last_polled and the
    conditional-GET headers for all feeds across all guilds so they re-fetch on
    the next cycle. seen_entries and fail_count are left intact, so no
    historical items are re-announced and backoff state survives. Used by the
    daily calibration task, which has no invoking guild. Returns the number of
    feeds reset.
    """
    cur = await _conn.execute(
        "UPDATE feeds SET last_polled = 0, etag = NULL, last_modified = NULL")
